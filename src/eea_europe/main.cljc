(ns eea_europe.main
  "Kotodama WASM entrypoint for the eea_europe clean-room actor (L4) — Clojure port.
  ...
  facts (`eea_europe.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "eea_europe")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"eea_europe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "eea_europe-compat" :tier tier :entities entities} 200])
...
(defn start! [] :eea_europe-compat/ready)
